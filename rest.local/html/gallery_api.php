<?php
// HTTP status codes https://datatracker.ietf.org/doc/html/rfc4918#page-79
...
function doGet()
{
	...
	if (isset($_GET['page'])) {
		if (is_numeric($_GET['page'])) {
			...
			if ($_GET['page'] == $get_page) {
				if ($get_page > 0) {
					$page = $get_page;
				} else $warn['page'] = "Illegal page number, default used";
			} else $warn['page'] = "Page number unrecognized, default used";
		} else $warn['page'] = "Invalid page number, default used";
	}
	if (isset($_GET['perpage'])) {
		if (is_numeric($_GET['perpage'])) {
			...
			if ($_GET['perpage'] == $get_page) {
				if ($get_page > 0) {
					$per_page = $get_page;
				} else $warn['perpage'] = "Illegal perpage value, default used";
			} else $warn['perpage'] = "Perpage value unrecognized, default used";
		} else $warn['perpage'] = "Invalid perpage value, default used";
	}
	// 3. SQL part
	...
	try {
		...
		while ($row = $ans->fetch(PDO::FETCH_ASSOC)) {
			if (isset($res[$row['id']])) {
				$res[$row['id']]['descr'][$row['iso639_1']] = $row['descr'];
			} else {
				$res[$row['id']] = [
...
function doPost()
{
	...
	*/
	...
	// Secondary : moving uploaded file
	// file extension:
